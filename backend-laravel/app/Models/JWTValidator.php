...
class JWTValidator
{
    ...
    public function isJwtValido($validade = 500)
    {
        if(!$this->isStringJwt()){
            return false;
        }
    ...
    public function objUsuario()
    {
        ...
        // $usuario->populaPerfil($usuarioToken['perfil']);
        return $usuario;
    }
    ...
    {
        $usuarioToken = $this->usuarioDoToken();
        if ($usuario = User::where('id', $usuarioToken['id'])->where('email', $usuarioToken['email'])->first()) {
            return $usuario;
        }
        return false;
    }
